...
const MCP_EXTENSION_SERVERS_ROOT = join(PACKAGES, 'mcp-extension-servers', 'servers')
...
// ─── Extension (packages/mcp-extension-servers) ───────────────────────────────
// Extension servers run in Chrome extension; use tab://izan-ext-{dir}
// Requires extension to be installed and running on izan.io

const extensionServers: Array<ServerMetadata & { url: string }> = []

if (existsSync(MCP_EXTENSION_SERVERS_ROOT)) {
  const entries = readdirSync(MCP_EXTENSION_SERVERS_ROOT, { withFileTypes: true })
  const serverDirs = entries
    .filter((e) => e.isDirectory())
    .filter((e) => existsSync(join(MCP_EXTENSION_SERVERS_ROOT, e.name, 'index.ts')))
    .filter((e) => existsSync(join(MCP_EXTENSION_SERVERS_ROOT, e.name, 'config.json')))
    .map((e) => e.name)
    .sort()

  for (const dir of serverDirs) {
    const configPath = join(MCP_EXTENSION_SERVERS_ROOT, dir, 'config.json')
    const contents = readFileSync(configPath, 'utf-8')
    const config = JSON.parse(contents) as ServerMetadata
    if (config.id && config.name && config.description && config.category) {
      extensionServers.push({
        ...config,
        url: `tab://izan-ext-${dir}`,
      })
    }
  }
}
...
export const BUILTIN_EXTENSION_SERVERS: BuiltinServerMetadata[] = ${extensionJson}
...
console.log(`[discover-builtin-servers] Wrote ${outPath} (${backendServers.length} backend + ${clientServers.length} client + ${extensionServers.length} extension)`)
