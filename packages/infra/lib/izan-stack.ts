...
function handler(event) {
  ...
  return request;
    ...
    // ─── S3 Deployment (MCP Tool Definitions) ───────────────────────────
    // Built-in automation tool JSONs served at /mcp-tools/*
    // These are fetched on demand when an agent references a built-in extension server
    const mcpToolsDir = path.join(MONOREPO_ROOT, 'packages', 'mcp-extension-servers', 'tool-definitions')
    new s3deploy.BucketDeployment(this, 'McpToolsDeployment', {
      sources: [s3deploy.Source.asset(mcpToolsDir)],
      destinationBucket: websiteBucket,
      destinationKeyPrefix: 'mcp-tools',
      distribution,
      distributionPaths: ['/mcp-tools/*'],
      cacheControl: [
        s3deploy.CacheControl.maxAge(cdk.Duration.hours(1)),
        s3deploy.CacheControl.sMaxAge(cdk.Duration.hours(24)),
      ],
    })

    // ─── S3 Deployment (Extension Server Registry) ───────────────────────
    // Pre-built extension server manifest served at /mcp-extension-servers/*
    // Generated by packages/mcp-extension-servers/scripts/generate-registry.ts
    const extensionRegistryDir = path.join(MONOREPO_ROOT, 'packages', 'mcp-extension-servers', 'built-in-registry')
    new s3deploy.BucketDeployment(this, 'ExtensionRegistryDeployment', {
      sources: [s3deploy.Source.asset(extensionRegistryDir)],
      destinationBucket: websiteBucket,
      destinationKeyPrefix: 'mcp-extension-servers',
      distribution,
      distributionPaths: ['/mcp-extension-servers/*'],
      cacheControl: [
        s3deploy.CacheControl.maxAge(cdk.Duration.hours(1)),
        s3deploy.CacheControl.sMaxAge(cdk.Duration.hours(24)),
      ],
