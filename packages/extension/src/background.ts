 ...
 * 4. WebSocket bridge client for external MCP connections
 */

import { getAllTools } from './code-tool-storage.js'
import { startBridgeClient, notifyToolsUpdated, isBridgeConnected } from './ws-client.js'
...
// Start the WebSocket bridge client to connect to external MCP clients
startBridgeClient(() => {
  // This runs synchronously - return cached tools
  return cachedTools
})

let cachedTools: CodeToolDefinition[] = []

// Load tools from storage and keep cache updated
...
// Listen for storage changes to keep tools in sync
chrome.storage.onChanged.addListener((changes) => {
  if (changes.izan_tools) {
    refreshToolCache().catch(() => {})
  }
})
...
async function handleSelectorExtract(tabId: number, selector: string, mode: 'list' | 'single'): Promise<void> {
  ...
  try {
    ...
    if (data?.error) {
      console.error(`[izan-ext] handleSelectorExtract error: ${data.error}`)
      ...
      console.log('[izan-ext] handleSelectorExtract step:', step)
    } else {
      console.log('[izan-ext] handleSelectorExtract: No elements found for this selector')
    }
  } catch (err) {
    console.error('[izan-ext] handleSelectorExtract exception:', err instanceof Error ? err.message : String(err))
  }
}
  ...
  try {
    ...
    function generateRelativeSelector(el: Element, container: Element, structuralOnly: boolean): string {
      ...
      while (cur && cur !== container) {
        var p: Element | null = cur.parentElement
        if (!p) break
        ...
        var sibs = Array.from(p.children).filter(function(c: Element) { return c.tagName === cur!.tagName })
...
function processRoleResult(value: unknown, mode: 'list' | 'single', roles?: string[], roleName?: string, roleIncludeChildren?: boolean): void {
  ...
  if (!v || v.error) {
    console.error(`[izan-ext] processRoleResult error: ${v?.error || 'Extraction failed'}`)
    return
  }
  if (!v.fields || (v.fields as unknown[]).length === 0) {
    console.error('[izan-ext] processRoleResult: No fields detected')
    return
  }
  const step = {
    action: 'extract' as const,
  ...
  console.log('[izan-ext] processRoleResult step:', step)
}
...
async function handleFullAccessibilitySnapshotForSidepanel(tabId: number): Promise<void> {
  try {
    ...
    console.log(`[izan-ext] handleFullAccessibilitySnapshotForSidepanel: ${formatted.length} chars`)
  } catch (e) {
    try {
      await chrome.debugger.sendCommand({ tabId }, 'Accessibility.disable').catch(() => {})
      await chrome.debugger.sendCommand({ tabId }, 'DOM.disable').catch(() => {})
    ...
    console.error('[izan-ext] handleFullAccessibilitySnapshotForSidepanel error:', e instanceof Error ? e.message : String(e))
  }
}
...
async function handleAccessibilityNeighborsForSidepanel(
  ...
  try {
    ...
    if ((result as { success: boolean }).success) {
      console.log(`[izan-ext] handleAccessibilityNeighborsForSidepanel: success`)
    } else {
      console.error(`[izan-ext] handleAccessibilityNeighborsForSidepanel error: ${(result as { error: string }).error}`)
    }
  } catch (e) {
    try { await cdpDetach(tabId) } catch {}
    console.error('[izan-ext] handleAccessibilityNeighborsForSidepanel error:', e instanceof Error ? e.message : String(e))
  }
}
...
async function handleRoleExtract(tabId: number, roles: string[], name: string, includeChildren: boolean): Promise<void> {
  try {
    ...
    if (allBackendNodeIds.length === 0) {
      ...
      console.error(`[izan-ext] handleRoleExtract: No elements found with role="${rolesStr}"${name ? ` and name="${name}"` : ''}`)
    ...
    console.error(`[izan-ext] handleRoleExtract error: ${message}`)
  }
}
...
// ─── Side Panel & Tool Test Messages ──────────────────────────────────────────

chrome.runtime.onMessage.addListener(
  (msg: { type: string; tool?: CodeToolDefinition; args?: Record<string, unknown> }, _sender, sendResponse) => {
    if (msg.type === 'izan-bridge-status') {
      sendResponse({ connected: isBridgeConnected() })
      return false
    }

    if (msg.type === 'izan-test-tool' && msg.tool && msg.args) {
      executeCodeTool(msg.tool, msg.args)
